...
public class Pano0110CreateStaticTourService extends BaseService {
  @Autowired
   ...
   * @throws Exception
   */
  public void doCreateStaticTour(Pano0110Form inForm) throws Exception {
    String outputExpositionId = inForm.selectedExpositionId;
    // 如果先前有发布成功的展览，先删除先前发布的文件夹
   ...
   * @throws Exception
   */
  public void creatXml(Pano0110Form inForm, String destPath) throws Exception {
    String xmlPath = destPath + "show.xml";
    ...
    onstartEvent.append("\r\n    ");
    // 2016/12/20非VR发布时，将webvr_enterbutton层设置为不可见，即不显示vr图标
    onstartEvent.append("set(layer[webvr_enterbutton].visible,false);");
    onstartEvent.append("\r\n        ");
  ...
  private void createSceneNodes(Pano0110Form _inForm, Document _xmldoc, Element _newRoot,
    ...
    for (int i = 0; i < panoramaList.size(); i++) {
      ...
      imageCubeElement.setAttribute("url",
          "panoramas/" + panoPanorama.panoramaId + "/panos_l/sphere_%s.jpg");
      ...
      mobileCubeElement.setAttribute("url",
          "panoramas/" + panoPanorama.panoramaId + "/panos_l/mobile_%s.jpg");
   ...
   * @throws Exception
   */
  public void createJs(Pano0110Form _inForm, String _destPath) throws IOException {
    String jsPath = _destPath + "framework/panorama/template/html/js/base.js";
    String jsFilePath = FwFileUtils.getAbsolutePath(jsPath);
    ...
    try {
      String buffer = createJsStr(_inForm.selectedExpositionId, " pano0110");
      fos = new OutputStreamWriter(new FileOutputStream(file), "UTF-8");
      fos.write(buffer.toString().toCharArray());
      fos.flush();
    } catch (Exception e) {
      e.printStackTrace();
    } finally {
   ...
   * 生成base.js文件字符串
   * 
   * @param _selectedExpositionId
   * @throws Exception
   */
  public String createJsStr(String _selectedExpositionId, String pageId) throws Exception {

    HashMap<String, Object> condition = Maps.newHashMap();
    condition.put("expositionId", _selectedExpositionId);
    ...
    List<PanoExposition01Model> txtImgList = pano0110Mapper.selectTxtImg(condition);
    StringBuffer buffer = new StringBuffer();
    Map<String, List<Map<String, String>>> jsonList =
        new HashMap<String, List<Map<String, String>>>();
    String newstr = "var hotspot_info = " + "\r\n";
    buffer.append(newstr);
    for (PanoExposition01Model dto : txtImgList) {
      if (!jsonList.containsKey(dto.hotspotId)) {
        jsonList.put(dto.hotspotId, new ArrayList<Map<String, String>>());
      }
      PanoMaterial hotspotMaterial = panoMaterial01Mapper.selectByPrimaryKey(dto.materialId);
      Map<String, String> obj = new HashMap<String, String>();
      obj.put("text", dto.textInfo);
      if (" pano0104".equals(pageId)) {
        String materialPath = MessageFormat.format(PanoConstantsIF.VAL_APP_SERVER_W,
            UserSessionUtils.getSessionId(), dto.materialPath);
        obj.put("img", FwFileUtils.getContextPath("/" + materialPath));
      } else {
        obj.put("img", "../../../../" + doGetMaterialFolderPath(hotspotMaterial.expositionId,
            _selectedExpositionId, dto.materialPath));
      }

      jsonList.get(dto.hotspotId).add(obj);
    }
    buffer.append(objectMapper.writeValueAsString(jsonList));
