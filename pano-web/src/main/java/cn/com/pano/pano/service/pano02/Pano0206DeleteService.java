...
public class Pano0206DeleteService extends BaseService {
  @Autowired
   ...
   * @throws Exception
   */
  public void doDelete(Pano0206Form _inForm) throws Exception {
    HashMap<String, Object> conditions = Maps.newHashMap();

    BigDecimal currentPanoSortKey = new BigDecimal(0);
    // 检索到当前全景图
    if (!ObjectUtils.isEmpty(_inForm.panoramaIdForDelete)) {
      // 1.删除其他场景中指向当前场景的热点下的全景图ID
      PanoPanorama panoPanorama =
          panoPanorama01Mapper.selectByPrimaryKey(_inForm.panoramaIdForDelete);
      ...
      if (panoPanorama != null) {
        ...
        conditions.put("panoramaId", _inForm.panoramaIdForDelete);
        ...
        if (!ObjectUtils.isEmpty(_inForm.expositionMapId)) {
          ...
          if (result != null && result.size() > 0) {
            for (PanoExpositionMapHotspot panoExpositionMapHotspot : result) {
              if (_inForm.panoramaIdForDelete.equals(panoExpositionMapHotspot.panoramaId)) {
                // 去掉热点下指向的全景图ID
        ...
        condition.put("panoramaId", _inForm.panoramaIdForDelete);
        ...
        String filePath = MessageFormat.format(PanoConstantsIF.VAL_PUBLIC_DIRECTORY_W_PANORAMA,
            _inForm.expositionId, _inForm.panoramaIdForDelete);
        FileServiceUtil.deletePublicStorageFolder(filePath);
