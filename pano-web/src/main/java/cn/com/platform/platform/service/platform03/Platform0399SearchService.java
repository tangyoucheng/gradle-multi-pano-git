 ...
 */
@Service
public class Platform0399SearchService extends BaseService {
  // 自定义SQL
  @Autowired
   ...
   * @return 提示信息
   */
  public EasyJson<PlatformCompany> doSearch(Platform0399Form inForm) {
    ...
    inForm.recordCount = platformzCompany01Mapper.selectCompanyCount(parameter);
    if (inForm.recordCount > 0) {
      // 每页数据
      inForm.companyList = platformzCompany01Mapper.selectCompanyInfo(parameter);
    }
   ...
   * @return 排序信息
   */
  private String getOrderInfo(Platform0399Form inForm) {
