...
<body>
    <div class="card-body">
        <div class="card">
            <div class="card-body">
                <form:form id="pano0208FormSearch" class="form-horizontal" modelAttribute="pano0208Form">
                    <input type="hidden" id="returnTargetIframe" name="returnTargetIframe"
                        value="${pano0208Form.returnTargetIframe}">
                    <input type="hidden" name="panoramaId" value="${pano0208Form.panoramaId}" />
                    <input type="hidden" name="expositionId" value="${pano0208Form.expositionId}" />
                    ...
                    <div class="form-group form-row">
                        <label class="col-form-label col-sm-1 text-left" for="txt_materialBelongType">素材归属种类</label>
                        <div class="col-sm-2">
                            <platform:bootstrap-select id="materialBelongType" name="materialBelongType"
                                selectedValue="${pano0208Form.materialBelongType}"
                                list="${pano0208Form.materialBelongTypeList}" className="form-control" blank="false" />
                        </div>
                        <label class="col-form-label col-sm-1 text-left" for="txt_hotspotTypeChoice">热点类别</label>
                        <div class="col-sm-2">
                            <platform:bootstrap-select id="hotspotTypeChoice" name="hotspotTypeChoice"
                                selectedValue="${pano0208Form.hotspotTypeChoice}"
                                list="${pano0208Form.hotspotTypeChoiceList}" className="form-control" blank="false" />
                        </div>
                    </div>
    ...
    <!-- Ajax提交用的Form -->
    <form:form id="pano0208FormAjaxSubmit" action="/" modelAttribute="pano0208Form">
        <input type="hidden" name="panoramaId" value="${pano0208Form.panoramaId}" />
        <input type="hidden" name="expositionId" value="${pano0208Form.expositionId}" />
    </form:form>
