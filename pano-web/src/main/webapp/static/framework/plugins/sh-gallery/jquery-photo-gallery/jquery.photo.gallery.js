...
$.extend({
	...
	initGallery : function(){
        var activeIndex = 0,
		 ...
		 // 创建对象
		var argImg = new Image();
		// 改变图片的src
		argImg.src = $(document).getUrlParam("imagePath");
		// 定时执行获取宽高
		var checkImgProp = function(){
			// 只要任何一方大于0
			// 表示已经服务器已经返回宽高
			if(argImg.width>0 || argImg.height>0){
				imgs.push({
					url: argImg.src, 
					imgHeight: argImg.height,
					imgWidth: argImg.width
				});
				...
				clearInterval(checkImgPropInterval);
			}
		};
		var checkImgPropInterval = setInterval(checkImgProp,40);	
