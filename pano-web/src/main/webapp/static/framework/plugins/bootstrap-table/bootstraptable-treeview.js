...
(function ($) {
    ...
    BootstrapTable.prototype.initRow = function(item, i, data, parentDom) {
        ...
        $.each(this.header.fields, function(j, field) {
            ...
            if (that.formHtml && typeof value_ === 'undefined') {
                return;
            }
    ...
    BootstrapTable.prototype.initBody = function (fixedScroll) {
        ...
        this.$selectItem.off('click').on('click', function (event) {
            ...
            // 子节点的勾选框
            row['checked'] = checked;
            // 取消子节点的勾选时，同时取消其父节点的勾选
            if (checked === false) {
            	var parent = getParent(row, that.options.data, that.options.treeId);
                $.each(parent, function (i, c) {
                	$.each(that.data, function (index, item) {
                        if (item[that.options.treeId] == c[that.options.treeId]) {
                            item.checked = checked ? true : false;
                            item.checkbox = checked ? true : false;
