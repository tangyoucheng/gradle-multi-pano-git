...
$.fn.overhang = function (arguments) {
  ...
  // Message
  var $message = $("<div class='overhang-message'></div>");
  $message.css("color", attributes.textColor);

  // Assign html or text
  if (attributes.html) {
    $message.html(attributes.message);
    if (attributes.detail.length > 0) {
        $message.html('');
        var detailInfo = '';
        jQuery.each(attributes.detail, function(i, item) {
            detailInfo = detailInfo + item;
        });
        $message.html(detailInfo);
    }
  } else {
    ...
    if (attributes.detail.length > 0) {
        $message.text('');
        jQuery.each(attributes.detail, function(i, item) {
            var $detailInfo = $("<div class='overhang-message'></div>");
            $detailInfo.text(attributes.upper ? item.toUpperCase() : item);
            $overhang.append($detailInfo);
        });
    }
  }
