...
## Features
  ...
  - Works with numbers with or without fraction digits in bases form 2 to 64 inclusive
...
*In all querys below, `var`, semicolons and `toString` calls are not shown.
...
and a base form 2 to 36 inclusive can be specified.
...
For advanced usage, multiple BigNumber constructors can be created, each with their own independent configuration which applies to all BigNumber's created form it.
...
To test a single method, use, for query
