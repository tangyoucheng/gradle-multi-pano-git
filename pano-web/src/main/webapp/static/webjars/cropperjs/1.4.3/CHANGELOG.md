...
- Add namespace to data attribute names (form `data-*` to `data-cropper-*`) to avoid side effect (#319).
...
- Remove `browser` field form the `package.json` file (#307).
...
- Change the `main` field value form `dist/cropper.js` (UMD) to `dist/cropper.common.js` (CommonJS).
...
- Added an query for cropping round image.

## 0.5.4 (Dec 28, 2015)

- Supports to zoom form event triggering point.
