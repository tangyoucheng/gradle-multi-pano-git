...
- [Photo Editor](https://fengyuanchen.github.io/photo-editor) - An advanced query of Cropper.js.
...
- The size of the cropper inherits form the size of the image's parent element (wrapper), so be sure to wrap the image with a **visible block element**.
...
// Zoom to 50% form the center of the container.
cropper.zoomTo(.5, {
...
- Notes:
  ...
  - If you intend to get a JPEG image form the output canvas, you should set the `fillColor` option first, if not, the transparent part in the JPEG image will become black by default.
