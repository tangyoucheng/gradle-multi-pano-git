...
$(function() {
    ...
    $("#btn_entry").click(function() {
        // 统一移除popover提示消息层
        $("#platformz030101Form").validate().destroy();
        // validate初始化
        $("#platformz030101Form").validate();
        // $("#txt_bankNameCn").attr('required', '');
        // $("#txt_bankNameEn").attr('required', '');
        // $("#txt_bankCode").attr('required', '');
        // 有验证错误时返回
        if (!$("#platformz030101Form").valid()) {
            return;
        }
        ...
        // 按钮
            ...
            var targetUrl = 'platformz030101/doEntry';
            var formData = form2js($("#platformz030101Form")[0]);
            $.ajax({
                type : 'post',
                data : formData,
                dataType : 'json',
