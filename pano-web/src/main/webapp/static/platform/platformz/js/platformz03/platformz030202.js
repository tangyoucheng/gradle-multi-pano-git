...
$(function() {
    ...
    $("#btn_update").click(function() {
        // 统一移除popover提示消息层
        $("#platformz030202Form").validate().destroy();
        // validate初始化
        $("#platformz030202Form").validate();
        // $("#txt_bankNameCn").attr('required', '');
        // $("#txt_bankNameEn").attr('required', '');
        // $("#txt_bankCode").attr('required', '');
        // 有验证错误时返回
        if (!$("#platformz030202Form").valid()) {
            return;
        }
        // 询问框
        ...
        // 按钮
            ...
            var targetUrl = 'platformz030202/doUpdate';
            var formData = form2js($("#platformz030202Form")[0]);
            $.ajax({
                type : 'post',
                traditional : true,
                data : formData,
