...
$(function() {
    ...
    $("#btn_entry").click(function() {
        // 统一移除popover提示消息层
        $("#platformz030201Form").validate().destroy();
        // validate初始化
        $("#platformz030201Form").validate();
        // $("#txt_bankNameCn").attr('required', '');
        // $("#txt_bankNameEn").attr('required', '');
        // $("#txt_bankCode").attr('required', '');
        // 有验证错误时返回
        if (!$("#platformz030201Form").valid()) {
            return;
        }
        // 询问框
        ...
        // 按钮
            ...
            var targetUrl = 'platformz030201/doEntry';
            var formData = form2js($("#platformz030201Form")[0]);
            $.ajax({
                type : 'post',
                data : formData,
                dataType : 'json',
