// 
$(function() {
    var tableAffairsIdInfo = $('#table-affairsIdInfo');

    // bootstrapTable初始化
    tableAffairsIdInfo.bootstrapTable({
    // locale : 'zh-CN'
    // height : 500
    });
    // toolbar的制御
    var selectionsLength = tableAffairsIdInfo.bootstrapTable('getSelections').length;
    $('#platformz0302Btn_delete').prop('disabled', !selectionsLength);

    var jqueryEvent = 'check.bs.table uncheck.bs.table load-success.bs.table '
            + 'check-all.bs.table uncheck-all.bs.table';
    tableAffairsIdInfo.on(jqueryEvent, function() {
        var selectionsLength = tableAffairsIdInfo.bootstrapTable('getSelections').length;
        $('#platformz0302Btn_delete').prop('disabled', !selectionsLength);
    });
    // 新建
    $("#platformz0302Btn_new").click(function() {
        var targetUrl = 'platformz030201/';
    ...
    // 删除
    $("#platformz0302Btn_delete").click(function() {
         // 选中表格数据ID
        var uniqueKeyArray = new Array();
        uniqueKeyArray = $.map($("#table-affairsIdInfo").bootstrapTable('getSelections'), function(row) {
            // 传入后台的唯一识别ID
        ...
        if (uniqueKeyArray != null && uniqueKeyArray != "") {
            ...
            // 按钮
                ...
                // 表单数据转换成JS对象
                var ajaxSubmitFormData = form2js($("#platformz0302FormAjaxSubmit")[0]);
                ...
                $.ajax({
                    ...
                    dataType : 'json',
                    url : getMemberContextPath('platformz0302/doDelete'),
    ...
    searchData();
    // 检索
    $("#platformz0302Btn_query").click(function() {
        // debugger; 检测
        searchData();
    });
...
function searchData() {
    // 先销毁表格
    $('#table-affairsIdInfo').bootstrapTable('destroy');
    // 初始化表格,动态从服务器加载数据
    $('#table-affairsIdInfo').bootstrapTable({
        url : getMemberContextPath('platformz0302/doSearch'), // 请求后台的URL（*）
        method : 'get', // 请求方式（*）
        ...
        queryParams : function queryParams(params) { // 设置查询参数
            // 表单数据转换成JS对象
            var formData = form2js($("#platformz0302FormSearch")[0]);
...
function doEdit(tableRowInfo) {
    var targetUrl = 'platformz030202/';
    targetUrl = targetUrl + '?affairsId=' + tableRowInfo.affairsId;
    window.top.layer.open({
        title : '编辑',
        type : 2,
        closeBtn : 1, // 显示关闭按钮
