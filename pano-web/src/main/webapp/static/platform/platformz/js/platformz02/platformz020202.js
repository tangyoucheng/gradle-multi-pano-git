...
$(function() {
    ...
    $("#btn_update").click(function() {
        // 统一移除popover提示消息层
        $("#platformz020202Form").validate().destroy();
        // validate初始化
        $("#platformz020202Form").validate();
        // $("#txt_bankNameCn").attr('required', '');
        // $("#txt_bankNameEn").attr('required', '');
        // $("#txt_bankCode").attr('required', '');
        // 有验证错误时返回
        if (!$("#platformz020202Form").valid()) {
            return;
        }
        ...
        // 按钮
            ...
            var targetUrl = 'platformz020202/doUpdate';
            var formData = form2js($("#platformz020202Form")[0]);
            $.ajax({
                type : 'post',
                data : formData,
                dataType : 'json',
