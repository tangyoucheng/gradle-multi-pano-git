...
$(function() {
    ...
    $("#btn_entry").click(function() {
        // 统一移除popover提示消息层
        $("#platformz020201Form").validate().destroy();
        // validate初始化
        $("#platformz020201Form").validate();
        // $("#txt_bankNameCn").attr('required', '');
        // $("#txt_bankNameEn").attr('required', '');
        // $("#txt_bankCode").attr('required', '');
        // 有验证错误时返回
        if (!$("#platformz020201Form").valid()) {
            return;
        }
        ...
        // 按钮
            ...
            var targetUrl = 'platformz020201/doEntry';
            var formData = form2js($("#platformz020201Form")[0]);
            $.ajax({
                type : 'post',
                data : formData,
                dataType : 'json',
