...
$(function() {
    ...
    $("#btn_entry").click(function() {
        // 统一移除popover提示消息层
        $("#platform039901Form").validate().destroy();
        // validate初始化
        $("#platform039901Form").validate();
        // $("#txt_bankNameCn").attr('required', '');
        // $("#txt_bankNameEn").attr('required', '');
        // $("#txt_bankCode").attr('required', '');
        // 有验证错误时返回
        if (!$("#platform039901Form").valid()) {
            return;
        }
        ...
        // 按钮
            ...
            var targetUrl = 'platform039901/doEntry';
            var formData = form2js($("#platform039901Form")[0]);
            $.ajax({
                type : 'post',
                data : formData,
                dataType : 'json',
