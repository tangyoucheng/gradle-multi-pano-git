...
var vr0209_loadHotspotFromVr0203_Interval;
var vr0209_loadHotspotFromVr0203_Interval_processFlag = false;

var load_vr0209_Interval;
var load_vr0209_Interval_processFlag = false;

var vr0209_loadLayer_Interval;
var vr0209_loadLayer_Interval_processFlag;

var vr0209ButtonsInterval;
var vr0209ButtonsInterval_processFlag = false;

var vr0209RecommendInterval;
var vr0209RecommendInterval_processFlag = false;
...
$(document).ready(function(){
    ...
    $("input[name='chkBox']").click(function(){
        reloadButtonsBar_vr0209();
    });
    if($('#commandTypeFromVr0105').val() == '' || $('#commandTypeFromVr0105').val() == null){
   ...
   // 保存处理
   $('#button-vr0209-confirm').click(function(){
       set0209LookAtForEdit(); 
       if($('#commandTypeFromVr0105').val() == 'buttons' ){
           //调用Vr0105保存工具条方法
           reloadButtonsBar_vr0209();
           ...
           imuiConfirm('是否保存修改?', '确认', function() {
               if($('#flag').val() == 'size'){
                   ...
                   changeHotspotSize(hotspotScale,$('#vr0203HotspotName').val());
               }else{
//                   if($('#commandTypeFromVr0105').val() == 'commonInfo' 
//                       && $('#recommendInfoText').val() =='' || $('#recommendInfoText').val() == null){
//                       imuiAlert("请输入推荐路线信息！");
//                       return false;
//                   }
                   var _ajaxUrl = getMemberContextPath('panoVr0209/doSaveHotspotScale');
                   $('#hotspotScale').val(hotspotScale);
//                   $('#recommendInfo').val($('#recommendInfoText').val());
                   $('#recommendInfo').val("推荐路线");
                   var param = $("#vr0209Form").serialize();
                   jQuery.post(_ajaxUrl, param, function(data){
                       if(CommonUtilJs.processAjaxSuccessAfter(data)){
                           
                           if(data == 'recommend_info'){
                               $("#back-to-vr0105").submit();
                           }
                           ...
                           if(data == 'true_guid_hotspot'){
                               $('#recommendHotspotIdBackTo0104').val($('#vr0209hotspotId').val());
                               $("#back-to-vr0104").submit();
                           }
   ...
   if(PanoConstants.VAL_HOTSPOT_TYPE_HOTSPOT_CHANGE_SCENE != $('#vr0209HotspotType').val()){
       $('#recommendTr').hide();
       $('#tooltipTr').hide();
       if($('#commandTypeFromVr0105').val() != 'commonInfo'){
...
//       _recommendInfo = $('#expoRecommendInfo').val();
   ...
   $('#recommendFlag').click(function(){
       var _krpano = document.getElementById("vr0209PanoId");
...
function doMakeVr0209krpano(){
    var vr0209Krpano = document.getElementById("vr0209videoPreviewKrpano");
    if (vr0209Krpano == null || undefined == vr0209Krpano.get){
        var xmlPath = $('#vr0209panoramaPath').val()
        embedpano({swf:PanoConstants.VAL_EMBEDPANO_SWF
                , id:"vr0209PanoId"
                , xml:xmlPath
                , target:"vr0209Pano",wmode:"opaque"
                , flash: "only" 
                , bgcolor:"#f5f5f5"
...
//区分浮动层大小调整，工具条自定义和热点大小调整的画面
function editPage_vr0209(){
    // 调整浮动层大小
    if($('#commandTypeFromVr0105').val() == 'flowInfo' ){
        vr0209_loadLayer_Interval = setInterval(function(){loadLayer();},500);
        $('#buttonslistDiv').hide();
...
//        $('#editRecommendInfoTr').hide();
    ...
    // 自定义工具条
    if($('#commandTypeFromVr0105').val() == 'buttons' ){
        vr0209ButtonsInterval = setInterval(function(){vr0209ShowCurrentButtonsBar()},1000);
        $('#sizeTr').hide();
        $('#recommendTr').hide();
        $('#sizeTitleDiv').hide();
        $('#editRecommendTitleDiv').hide();
//        $('#editRecommendInfoTr').hide();
    ...
    if($('#commandTypeFromVr0105').val() == 'commonInfo' ){
        ...
        $('#sizeTitleDiv').hide();
        $('#tooltipTr').hide();
        $('#vr0209Pano').css("height","280px");
        vr0209RecommendInterval = setInterval(function(){loadRecommendInfo_vr0209();},500);
        return false;
    }
...
//        $('#editRecommendInfoTr').hide();
        ...
        $('#sizeTitleDiv').hide();
        $('#sizeTr').hide();
        $("#button-vr0209-confirm").hide();
        musicInterval = setInterval(function(){loadMusicHotspotFromVr0203();},500);
        return false;
    }
    ...
    // 分0203和0104操作两种情况
    if($('#flag').val() == 'size'){
        vr0209_loadHotspotFromVr0203_Interval = setInterval(function(){loadHotspotFromVr0203();},500);
        $('#buttonslistDiv').hide();
        $('#buttonsTitleDiv').hide();
//        $('#editRecommendInfoTr').hide();
        $('#editRecommendTitleDiv').hide();
        $('#sizeTitleDiv').show();
        $('#sizeTr').show();
        $("#button-vr0209-confirm").show();
    }else {
        load_vr0209_Interval = setInterval(function(){load_vr0209();},1000);
        $('#buttonslistDiv').hide();
        $('#buttonsTitleDiv').hide();
//        $('#editRecommendInfoTr').hide();
        $('#editRecommendTitleDiv').hide();
...
//显示现有状态下的工具条样式
function vr0209ShowCurrentButtonsBar(){
    
    var _krpano = document.getElementById("vr0209PanoId");
    if (_krpano == null || undefined == _krpano.get || vr0209ButtonsInterval_processFlag){
        return false;
    }
    vr0209ButtonsInterval_processFlag = true;
    ...
    clearInterval(vr0209ButtonsInterval);
    vr0209ButtonsInterval_processFlag = false;
}

//展览工具条选中的按钮名称组合
var _selectedButtons = '';
//设定工具条的显示
function reloadButtonsBar_vr0209(){
    var _krpano = document.getElementById("vr0209PanoId");
    ...
    //坐标变量
    var _positionX = 0;
    $('#vr0209ListDataTable').find('input[type="checkbox"]').each(function(){
...
// 加载热点
function load_vr0209(){
    var _krpano = document.getElementById("vr0209PanoId");
    if (_krpano == null || undefined == _krpano.get
            || load_vr0209_Interval_processFlag
            || $('#hotspotImageInfoJson').val() == ''){
        return false;
    }
    load_vr0209_Interval_processFlag = true;
    var newspot = 'v_' + $('#vr0209hotspotId').val();
    ...
    if(hotspotImageInfoObject == null){
        clearInterval(vr0209_loadHotspotFromVr0203_Interval);
        vr0209_loadHotspotFromVr0203_Interval_processFlag = false;
        return false;
    }
    
    var hotspotStyle = null;
    var hotspotStyleOnloaded = null;
    var _url = getMemberContextPath(''+hotspotImageInfoObject.hotspotImagePath);
...
//           //推荐路线信息为文字时
//           _krpano.set('plugin[recommedInfoPlugin].html',_recommendInfo);
//           _krpano.set('plugin[recommedInfoPlugin].visible',"true");
//           _krpano.set('plugin[recommedInfoPlugin].parent',"hotspot[" + newspot + "]");
//           
...
//           _krpano.call("stopdelayedcall('"+newspot+"');");
   ...
   clearInterval(load_vr0209_Interval);
   load_vr0209_Interval_processFlag = false;
}

// 由热点编辑画面呼出的热点大小调整，读取热点
function loadHotspotFromVr0203(){
    var _krpano = document.getElementById("vr0209PanoId");
    if (_krpano == null || undefined == _krpano.get 
            || vr0209_loadHotspotFromVr0203_Interval_processFlag
            || $('#hotspotImageInfoJson').val() == ''){
        return false;
    }
    vr0209_loadHotspotFromVr0203_Interval_processFlag = true;
    
    var newspot = $('#vr0203HotspotName').val();
    _krpano.call('addhotspot('+newspot+')'); 
    // 初期显示时保持热点图片的原始尺寸的设定
    _krpano.set('hotspot['+newspot+'].zoom',true);
    var ath = $('#vr0203HotspotAth').val();
    var atv = $('#vr0203HotspotAtv').val();
    _krpano.set('hotspot['+newspot+'].ath',ath);
    _krpano.set('hotspot['+newspot+'].atv',atv);
    _krpano.set('hotspot['+newspot+'].zorder',"2");
    if(hotspotScale != "" && hotspotScale != null){
...
function loadMusicHotspotFromVr0203(){
    var _krpano = document.getElementById("vr0209PanoId");
    if (_krpano == null || undefined == _krpano.get
    ...
    var newspot_1 = $('#vr0203HotspotName').val()+"_1";
    var newspot_2 = $('#vr0203HotspotName').val()+"_2";
    ...
    if(firstImageInfoObject != null 
        ...
        var gifstyle = 'gifstyle' + $('#vr0209hotspotId').val().split('v_').join('')+"_1";
        _krpano.call('addstyle('+gifstyle+')');
        _krpano.set('style['+gifstyle+'].name',gifstyle);
    ...
    if(secondImageInfoObject != null 
        ...
        var gifstyle_2 = 'gifstyle' + $('#vr0209hotspotId').val().split('v_').join('')+ "_2";
    ...
    //音频热点的url切换
    switchFlg = true;
    _krpano.set('hotspot['+newspot_1+'].onclick','js(changeUrl_vr0209('+newspot_1+','+newspot_2+'))'); 
    _krpano.set('hotspot['+newspot_2+'].onclick','js(changeUrl_vr0209('+newspot_1+','+newspot_2+'))');     
...
var switchFlg;
//更换音频热点的图片
function changeUrl_vr0209(newspot_1,newspot_2){
    var _krpano = document.getElementById("vr0209PanoId");
    if(switchFlg){
...
//在编辑推荐路线点信息时预览效果
function loadRecommendInfo_vr0209(){
    
    var _krpano = document.getElementById("vr0209PanoId");
    if (_krpano == null || undefined == _krpano.get || vr0209RecommendInterval_processFlag){
        return false;
    }
    vr0209RecommendInterval_processFlag == true;
    ...
    clearInterval(vr0209RecommendInterval);
    vr0209RecommendInterval_processFlag = false;
}
...
function loadLayer(){
    var _krpano = document.getElementById("vr0209PanoId");
    if (_krpano == null || undefined == _krpano.get || vr0209_loadLayer_Interval_processFlag){
        return false;
    }
    vr0209_loadLayer_Interval_processFlag = true;
    ...
    _krpano.set('layer['+newlayer+'].x', $('#vr0209layerX').val());
    _krpano.set('layer['+newlayer+'].y', $('#vr0209layerY').val());
    _krpano.set('layer['+newlayer+'].scale',hotspotScale);
    //如果是图片浮动信息
    if($('#vr0209flowFileType').val() == PanoConstants.VAL_MATERIAL_TYPE_FLOW_INFO_IMAGE){
        _krpano.set('layer['+newlayer+'].url',$('#vr0209flowFilePath').val());
    }else{
    //如果是文字浮动信息    
        _krpano.set('layer['+newlayer+'].url','%SWFPATH%/plugins/textfield.swf');
        _krpano.set('layer['+newlayer+'].html',$('#vr0209flowFileInfo').val());
    ...
    clearInterval(vr0209_loadLayer_Interval);
    vr0209_loadLayer_Interval_processFlag = false;
}
