var vr0105Interval = setInterval(function(){showMiniMap()},1000);
var vr0105MapHotspotInterval;
var vr0105ButtonsInterval = setInterval(function(){showCurrentButtonsBar()},1000);
var vr0105FlowInfoInterval = setInterval(function(){showCurrentFlowInfoLayer()},1000);
var vr0105RadarInterval = null;
...
$(document).ready(function(){
    ...
    $('#button-set-tool-confirm').click(function(){
        eval("$('#vr0105SetToolFinish').imuiDialog('close')");
        $('#vr0105Form').submit();
    });
    ...
    //雷达角度左旋
    $('#left').click(function(){
        var krpano = document.getElementById("vr0105KrpanoSWFObject");
        offsetNum-=10;
    ...
    $('#set_radar').click(function(){
        if(vr0105RadarInterval==null){
            vr0105RadarInterval = setInterval(function(){doRadarByTime()},500);
        }
    });
    ...
    $("#set_flow_info").click(function(){
        //打开浮动信息的选择提示框
        eval("$('#vr0105FlowInfoCommandChoice').imuiDialog('open')");
    });
    ...
    $('#button-edit-layer-size').click(function(){
        ...
        var _ajaxUrl = getMemberContextPath('panoVr0105/doCheckFlowInfoLayer');
            var param = $("#vr0105Form").serialize();
            jQuery.post(_ajaxUrl, param, function(data){
                if(CommonUtilJs.processAjaxSuccessAfter(data)){
                    if(data == 'true'){
                        callVr0209();
                    }else{
                        eval("$('#vr0105FlowInfoEidtChoice').imuiDialog('close')");
                        imuiAlert("请先保存浮动效果");
                    }
                }
        });
...
function doRadarByTime() { 
       var krpano = document.getElementById("vr0105KrpanoSWFObject");
        ...
        clearInterval(vr0105RadarInterval);
        vr0105RadarInterval=null;
        
        imuiConfirm('是否保存当前热点的雷达角度', '确认', function() { 
            var _ajaxUrl = getMemberContextPath('panoVr0105/doSaveRadarHeading');
            ...
            jQuery.post(_ajaxUrl, param, function(data){
                if(CommonUtilJs.processAjaxSuccessAfter(data)){
                    if(data == 'false'){
                        ...
                        eval("$('#vr0105Finish').imuiDialog('open')");
...
function setCommonInfo(){
    ...
    $("#editButtonsOrFlowInfo").imuiPageDialog({
        ...
        height: 600,
        parameter: {
            LastedHotspotIdFrom0105: $('#vr0105lastHotspotId').val(),
            currentHotspotIdFrom0105: $('#vr0105CurrentHotspotId').val(),
            commandTypeFromVr0105: 'commonInfo',
...
function openIc0109(){
    ...
    close: function(event, ui) {
        //关闭画面时，停止0109音乐
        var ic0109newVideokrpano1 = document.getElementById("newPano1Id");
        if (ic0109newVideokrpano1 != null && undefined != ic0109newVideokrpano1.get){
            ic0109newVideokrpano1.call('videoplayer_close();');
        }
        var ic0109newVideokrpano2 = document.getElementById("newPanoId");
        if (ic0109newVideokrpano2 != null && undefined != ic0109newVideokrpano2.get){
            ic0109newVideokrpano2.call('videoplayer_close();');
        }
        var ic0109oldVideokrpano = document.getElementById("oldPanoId");
        if (ic0109oldVideokrpano != null && undefined != ic0109oldVideokrpano.get){
            ic0109oldVideokrpano.call('videoplayer_close();');
        }
...
function showCurrentButtonsBar(){
    ...
    timeFlag = false;
    clearInterval(vr0105ButtonsInterval);
}
...
function doSaveButtonsBar(_selectedButtons){
    imuiConfirm('是否保存当前工具条状态', '确认', function() { 
        ...
        var _ajaxUrl = getMemberContextPath('panoVr0105/doSaveButtonsBar');
        var param = $("#vr0105Form").serialize();
        jQuery.post(_ajaxUrl, param, function(data){
            if(CommonUtilJs.processAjaxSuccessAfter(data)){
                if(data == 'true'){
                    eval("$('#vr0105SetToolFinish').imuiDialog('open')");
                }
            }
        });
    });
...
function closeDialog(){
    eval("$('#vr0105Finish').imuiDialog('close')");
    eval("$('#vr0105SetLayerFinish').imuiDialog('close')");
}
...
function showMiniMap(){
    ...
    //读取导航图上热点
    vr0105MapHotspotInterval = setInterval(function(){loadMapHotspot()},1000)
    clearInterval(vr0105Interval);
    timeFlag = false;
}

//呼出选择图片浮动效果素材的画面
function editFlowInfo_Image(){
    eval("$('#vr0105FlowInfoCommandChoice').imuiDialog('close')");
...
function loadMapHotspot(){
    ...
    $(spotInfoList).each(function(index, recordData){
        ...
        krpano.set('layer['+newlayer+'].onclick','activatespot('+newlayer+',0);js(showMiniMapHotspotInfo(get(name));)');
        krpano.set('layer['+newlayer+'].keep',true);
    });
    if ($('#selectedHotspotId').val() != undefined
            && $('#selectedHotspotId').val().length != 0) {
    ...
    clearInterval(vr0105MapHotspotInterval);
}
...
function showMiniMapHotspotInfo(_hotspotName,_radarHeading){
    ...
    var _ajaxUrl = getMemberContextPath('panoVr0105/doCheckHostspotInfo');
        ...
        if(CommonUtilJs.processAjaxSuccessAfter(data)){
            ...
            if (data !='' && data != null) {
                ...
                if (recordData.panoramaPath != '' && recordData.panoramaPath != null) {
                    ...
                    //设定地图热点和雷达角度
                    vr0105MapHotspotInterval = setInterval(function(){loadMapHotspot()},1000)
                    $('#vr0105title').html(recordData.panoramaName);
                }
            }else{
...
function callAddPanoramaLink(_selectedHotspotId){
    $("#addPanorama").imuiPageDialog({
        url: getMemberContextPath('panoVr0307'),
        title: '为热点添加场景图',
        modal: true,
        width: 820,
        height: 600,
...
function showCurrentFlowInfoLayer(){
    ...
    if($('#flowInfoFileId').val() != null && $('#flowInfoFileId').val() != ''){
        ...
        krpano.set('layer['+newlayer+'].ondown','draglayer();js(vr0105LayerDraglayer());');
        ...
        //如果是文字浮动信息    
            ...
            krpano.set('layer['+newlayer+'].css','text-align:center; font-family:Arial; font-weight:bold; font-size:'+parseFloat($('#flowInfoFileScale').val())*36+'px;');
            krpano.set('layer['+newlayer+'].scalechildren',true);
        }
        krpano.set('layer['+newlayer+'].backgroundalpha',"0");
        krpano.set('layer['+newlayer+'].height',"auto");
        krpano.set('layer['+newlayer+'].width',"auto");
        krpano.set('layer['+newlayer+'].onclick','js(vr0105LayerClick(get(name)));');
    }
    
    clearInterval(vr0105FlowInfoInterval);
}


//选择浮动信息素材后，显示浮动信息层
function vr0105ShowFlowInfoLayer(_flowInfoFileId,_filePath,_flowInfoType,_flowTextInfo){
    var krpano = document.getElementById("vr0105KrpanoSWFObject");
    //移除旧的layer
    ...
    //如果是文字浮动信息    
        ...
        krpano.set('layer['+newlayer+'].css','text-align:center; font-family:Arial; font-weight:bold; font-size:36px;');
        krpano.set('layer['+newlayer+'].scalechildren',true);
    }
    krpano.set('layer['+newlayer+'].backgroundalpha',"0");
    krpano.set('layer['+newlayer+'].height',"auto");
...
var layerName;
//浮动层点击事件
function vr0105LayerClick(_layerName) {
    eval("$('#vr0105FlowInfoEidtChoice').imuiDialog('open')");
    layerName = _layerName;

}
//浮动层拖拽事件
function vr0105LayerDraglayer() {
...
function deleteLayer(){
    ...
    imuiConfirm('是否移除当前浮动信息层', '确认', function() { 
        ...
        var _ajaxUrl = getMemberContextPath('panoVr0105/doDeleteFlowInfo');
        var param = $("#vr0105Form").serialize();
        jQuery.post(_ajaxUrl, param, function(data){
            if(CommonUtilJs.processAjaxSuccessAfter(data)){
                if(data == 'true'){
                    eval("$('#vr0105SetLayerFinish').imuiDialog('open')");
                }else{
                    imuiAlert("操作失败");
                }
            }
...
function saveFlowInfoOffset(){
    ...
    imuiConfirm('是否保存当前浮动信息层的位置', '确认', function() { 
        var _ajaxUrl = getMemberContextPath('panoVr0105/doSaveFlowInfoOffset');
        var param = $("#vr0105Form").serialize();
        jQuery.post(_ajaxUrl, param, function(data){
            if(CommonUtilJs.processAjaxSuccessAfter(data)){
                if(data == 'true'){
   ...
   setLookAtForEdit();
    $("#editButtonsOrFlowInfo").imuiPageDialog({
        ...
        parameter: {
            ...
            positionAthForEdit: positionAth,
            positionAtvForEdit: positionAtv,
            commandTypeFromVr0105: 'flowInfo',
            expositionIdForVr0209:$('#expositionId').val(),
            panoramaIdForVr0209: $('#panoramaId').val(),
            vr0209flowFileId: $('#flowInfoFileId').val(),
            vr0209flowFileType: $('#flowInfoType').val(),
            vr0209flowFilePath: $('#flowInfoFilePath').val(),
            vr0209flowFileInfo: $('#flowInfoFileInfo').val(),
