var vr0107Interval = setInterval(function(){showMiniMap()},1000);
var vr0107HotspotInterval = setInterval(function(){loadMapHotspot()},1000);
/**
 * 场景地图上热点的操作
 */
...
function showMiniMap(){
    ...
    var krpano = document.getElementById("vr0107KrpanoSWFObject");
...
function addMapHotspot(){
    ...
    krpano.set('layer['+newlayer+'].onclick','js(vr0107RemoveLayer(get(name)))');
}
//返回方法
function vr0107DoBack() {
    $("#back-form").submit();
}
//删除层
function vr0107RemoveLayer(_hotspotName) {
...
function loadMapHotspot(){
    ...
    clearInterval(vr0107HotspotInterval);
}
...
function saveMapHotspot(){
    ...
    var _ajaxUrl = getMemberContextPath('panoVr0107/doSave');
    var param = $("#vr0107Form").serialize();
    jQuery.post(_ajaxUrl, param, function(data){
        if(CommonUtilJs.processAjaxSuccessAfter(data)){
        	eval("$('#vr0107Finish').imuiDialog('open')");
        }
    });
