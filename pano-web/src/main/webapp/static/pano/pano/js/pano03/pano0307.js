...
$(document).ready(function() {
    ...
    $("#btn_choose").click(function() {
        ...
        // 按钮
            ...
            // 表单数据转换成JS对象
            var ajaxSubmitFormData = form2js($("#pano0307FormAjaxSubmit")[0]);
            ...
            $.ajax({
                ...
                dataType : 'json',
                url : getMemberContextPath('pano0307/doEntry'),
...
function searchData() {
    ...
    // 初始化表格,动态从服务器加载数据
    $('#table-panorama-info').bootstrapTable({
        url : getMemberContextPath('pano0307/doSearch'), // 请求后台的URL（*）
        method : 'get', // 请求方式（*）
        ...
        queryParams : function queryParams(params) { // 设置查询参数
            // 表单数据转换成JS对象
            var ajaxFormData = form2js($("#pano0307FormAjaxSubmit")[0]);
...
function doPreview(tableRowInfo) {
    layer.open({
        id : 'LAY_pano0307OpenLayer', // 设定一个id，防止重复弹出
        title : '场景预览', // false不显示标题栏
        type : 1, // 0（信息框，默认）1（页面层）2（iframe层）3（加载层）4（tips层）
        area : [ '360px', '360px' ], // 宽高
        content : $('#pano0307OpenLayer'), // 这里content是一个DOM，注意：最好该元素要存放在body最外层，否则可能被其它的相对元素所影响
        success : function(layero, index) {

            removepano("pano0307KrpanoNewObject");

            // 引擎配置文件路径
            var xmlfilePath = "pano0307/";
