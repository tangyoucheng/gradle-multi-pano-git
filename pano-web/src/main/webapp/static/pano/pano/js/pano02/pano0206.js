...
$(document).ready(function(){
    ...
    $("#search-button").click(function() {
          var _ajaxUrl = getMemberContextPath('pano0206/doSearch');
          var param = $("#ic0206Form").serialize();
    ...
    //返回处理
    $('#ic0206-back-button').click(function(){
       $('#backToIc0104PanoramaId').val($('#ic0206PanoramaId').val());
    ...
    $('#confirm-button').click(function(){
        imuiConfirm('是否保存当前场景序号状态?', '确认', function() {
            ...
            var _sortKeys = [];
            $("#ic0206ListDataTable tbody tr").each(function(){
            ...
            var _ajaxUrl = getMemberContextPath('pano0206/doSaveSortKey');
            var param = $('#ic0206Form').serialize();
            jQuery.post(_ajaxUrl, param, function(data){
            	 CommonUtilJs.removeMask();
                if(CommonUtilJs.processAjaxSuccessAfter(data)){
//                    $('#backToIc0104PanoramaId').val($('#ic0206PanoramaId').val());
                    $("#ic0206-refresh-from-edit-sortkey").submit();
                }
            });
        });
    });
...
function setTrColor(_selectedPanoramaId) {
    ...
    $('#ic0206ListDataTable tbody td').each(function(){
...
function editPanorama(_panoramaId) {
    ...
    $("#updatePanoramaPage").imuiPageDialog({
        url: getMemberContextPath('pano0202'),
        title: '场景信息编辑',
        modal: true,
        width: 1000,
        height: 700,
        parameter: {
            ic0202TheLastedSceneHotspotId: $('#ic0104LastedSceneHotspotId').val(),
            ic0202CurrentRecommendHotspotId: $('#ic0104CurrentHotspotId').val(), 
            flagFromParentPage: 'ic0206',
        ...
        close: function(event, ui) {
            //编辑画面关闭时，删除临时文件
            var _ajaxUrl = getMemberContextPath('pano0202/doDeleteTempFile');
            var param = 'panoramaId='+$("#ic0202panoramaId").val();
            jQuery.post(_ajaxUrl, param, function(data){
                if(CommonUtilJs.processAjaxSuccessAfter(data)){
                }
            });
...
function doPreview(_panoramaPath){
    if (_panoramaPath != null && _panoramaPath != '') {
        embedpano({swf:PanoConstants.VAL_EMBEDPANO_SWF
            , id:'ic0206KrpanoNewObject'
            , xml:_panoramaPath
            , target:"ic0206PreviewPano",wmode:"opaque"
            , flash: "only" 
            , passQueryParameters:true});
            
        $("#ic0206PreviewPano").css('display','block');
        eval("$('#ic0206imagePreview').imuiDialog('open')");
    }
}
...
function createListDataTable(data) {
    if(CommonUtilJs.processAjaxSuccessAfter(data)){
        ...
        if(data.indexOf("指定条件的数据不存在。请改变索条件后再检索。") != -1){
            ...
            $("#ic0206ListDataTable tbody").html('');
            $('#confirmDiv').hide();
            return false;
        }
        $('#confirmDiv').show();
        var ic0206JsForm = JSON.parse(data);
        // 刷新一览数据
        var tbodyHtml = "";
        $(ic0206JsForm.panoramaInfo).each(function(index, item) {
        ...
        $("#ic0206ListDataTable tbody").html(tbodyHtml);
    }
}
