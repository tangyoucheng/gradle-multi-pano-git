var ic0106Interval = setInterval(function(){showMiniMap()},1000);
/**
 * 全景图地图编辑操作
 */
...
function addMiniMap(){
    $("#addMiniMap").imuiPageDialog({
        ...
        height: 700,
        parameter: {
            lasthotspotIdFrom0106: $('#ic0106LasthotspotId').val(),
            currenthotspotIdFrom0106 : $('#ic0106CurrenthotspotId').val(),
...
//呼出为展览选定地图的页面
function setMiniMap(){
    $("<div id='setMiniMap'></div>").imuiPageDialog({
        ...
        parameter: {
            ...
            expositionId: $("#expositionId").val(),
            expositionName: $("#expositionName").val(),
            expositionMapName: $("#ic0106expositionMapName").val(),
            existedExpositionMapId: $("#expositionMapId").val(),
            panoramaId:$("#panoramaId").val()
        },
...
function deleteMiniMap(){
        imuiConfirm('是否删除导航图?', '确认', function() {
            ...
            var _ajaxUrl = getMemberContextPath('pano0106/doDelete');
            var param = $("#ic0106Form").serialize();
            jQuery.post(_ajaxUrl, param, function(data){
                if(CommonUtilJs.processAjaxSuccessAfter(data)){
                	eval("$('#ic0106DeleteFinish').imuiDialog('open')");
                }
            });
        });
}
...
function showMiniMap(){
    ...
    var krpano = document.getElementById("ic0106KrpanoSWFObject");
...
//        krpano.set('layer['+newlayer+'].ondown','draglayer();');
    ...
    clearInterval(ic0106Interval);
}

//返回方法
function ic0106DoBack() {
    $("#back-form").submit();
}
