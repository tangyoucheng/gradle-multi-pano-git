...
) engine=innodb default charset=utf8 comment '存储每一个已配置的 jobDetail 的详细信息';
...
) engine=innodb default charset=utf8 comment '存储简单的 Trigger，包括重复次数，间隔，以及已触发的次数';
...
) engine=innodb default charset=utf8 comment '以 Blob 类型存储存放日历信息， quartz可配置一个日历来指定一个时间范围';
...
) engine=innodb default charset=utf8 comment '存储少量的有关 Scheduler 的状态信息，假如是用于集群中，可以看到其他的 Scheduler 实例';
